...
namespace Rendering {
    ...
    {
        // Used to show warnings when the attribute exists
    ...
    void Material::set_shader_program(const Shared<ShaderProgram>& shader)
    {
    ...
    Shared<MaterialProperty>& Material::get_property(const std::string& name)
    {
        for (auto& prop : _properties)
            if (prop->get_name() == name)
                return prop;
